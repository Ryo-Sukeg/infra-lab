...
### Windows に Linux DNSサーバの FQDN を反映させる
DNS サーバ側設定  
`/etc/named.conf` ファイルの `options {  allow-query {` に Windows のIPアドレスを追記 → DNSサーバ再起動

Windows 側設定  
`コントロールパネル` → `ネットワークとインターネット` → `ネットワークと共有センター` → `アダプターの設定の変更` → 使用中のネットワークアダプタを右クリック（VirtualBox …）→ `プロパティ` → `インターネット プロトコル バージョン４(TCP/IPv4)` を選択 → `プロパティ` をクリック → `次の DNS サーバーのアドレスを使う` を選択してIPを入力（例：優先DNSサーバー：192.168.56.101）→ OK → 閉じる → PC再起動  
DNSサーバが正しく設定されたか確認  
コマンドプロンプトで実行：ipconfig /all  
設定したイーサネットアダプタのDNSサーバー欄に設定が反映されているのを確認  
名前解決テスト  
コマンドプロンプトで以下内容を確認  
正引き：nslookup rhel.lab.lan  
逆引き：nslookup 192.168.56.x  
